...
// InitCert initializes certificates with the given data directory
func InitCert() error {
	dataDir := config.GetConfig().DataDir
	certManager = NewCertManager(dataDir)

	if err := certManager.InitializeAll(); err != nil {
		logger.Error("Failed to initialize certificates: %v", err)
		return err
	}
